...
abstract class base_controller
{
    ...
    /**
     * Segurança: Whitelist de extensões permitidas.
     * Impede a execução de binários ou scripts maliciosos fora do escopo da IDE.
     */
    ...
    protected function is_extension_allowed($filename)
    {
        ...
        // Permite arquivos técnicos essenciais e placeholders de pasta
        $base = basename(str_replace('\\', '/', $filename));
        if ($base === '.placeholder' || $base === '.htaccess' || $base === 'changelog.txt')
        {
            return true;
        }
    ...
    protected function sanitize_rel_path($path)
    {
        ...
        // Bloqueia tentativas de subir níveis de diretório
        if ($path === '..' || strpos($path, '../') !== false || strpos($path, './') === 0 || strpos($path, '..') !== false)
        {
            return '';
        }

        // Remove a barra inicial para manter o path relativo ao projeto
        return ltrim($path, '/');
    }
     ...
     * Valida se o projeto existe e pertence ao usuário logado.
